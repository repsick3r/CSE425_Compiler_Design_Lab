...
void printFinal(){
    ...
    cout << escape << color6 << revbeautify("Matched") << escape << aum << "       " << escape << color9 << beautify("Stack") << escape << aum << "      " << escape <<
 color8 << beautify("Input") << escape << aum << "      " << escape << color11 << revbeautify("Action") << escape << aum << "\n";
    cout << escape << color12 << "--------------------------------------------------------------------------------------------------------------------" << escape << aum << "\n";
    for(int i=0; i<finalPrint.size(); i++){
        cout << escape << color6 << revbeautify(finalPrint[i][0]) << escape << aum << "       " << escape << color9 <<  beautify(finalPrint[i][1]) << escape << aum << "      " << escape << color8 <<  beautify(finalPrint[i][2]) << escape << aum << "      " << escape << color11  << revbeautify(finalPrint[i][3]) << escape << aum << "\n";
    }
}
...
int main(int argc, char *argv[]){
   ...
   // the input file is given as a argument to the command line which is read for input.
    ...
    cout << escape << color11 << inputs.size() << " TestCases" << escape << "[0m" << "\n";
    cout << escape << color11 << "Steps are printed whenever we are able to accept the input" << escape << "[0m" << "\n"; 
    for(int tc=0; tc < inputs.size(); tc++){
        ...
        if(input != "notValid"){
            ...
            if(!globalTruth){
                cout << escape <<  color6  << "This input is not accepted by the grammar : " << escape << "[0m " << escape << color9 << raw_input << escape << "[0m" << "\n";
            }
            else{
                printFinal();
                cout << "\n";
                cout << escape <<  color6  << "Input   : " << escape << "[0m " << escape << color9 << raw_input << escape << "[0m" << "\n";
                cout << escape <<  color6  << "Matched : " << escape << "[0m " << escape << color9 << raw_input << escape << "[0m" << "\n";
            }
        }
        else{
            cout << "Invalid Input\n";
